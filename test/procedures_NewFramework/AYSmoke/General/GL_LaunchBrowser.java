...
public class GL_LaunchBrowser extends CommonSeleniumActions implements OR {
	...
	public void GL_LaunchBrowser() throws Exception {
		try {
			...
			if ((url.contains("app")) && (url.contains("aybusqa"))) {
				if (url.contains("ApplicantConnectLogin") && url.contains("aybusqa&facelift") || url.endsWith("icu&facelift=true")) {
				...
				if (environment.equalsIgnoreCase("UKQA")) {
