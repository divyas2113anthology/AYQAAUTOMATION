...
//			writeFailure("Given Test Data["+yesno+"] either Incorrect or has not be Scripted ");
			...
			if (!description.equals("")) {
				Reporter.log("Step 1 - Enter the description as ["+description+"]");
				sendKeys(attributeName_cssselector, PA_Description, description);
			}
			if (!printeddate.equals("")) {
				Reporter.log("Step 1 - Perform the operation to check the checkbox as ["+printeddate+"]");
			...
			if (!duplexprinting.equals("")) {
				Reporter.log("Step 1 - Perform the operation to check the checkbox as ["+duplexprinting+"]");
			...
			if (!executepdfrule.equals("")) {
				Reporter.log("Step 1 - Perform the operation to check the checkbox as ["+executepdfrule+"]");
			...
			if (!excludeuploaddocument.equals("")) {
				Reporter.log("Step 1 - Perform the operation to check the checkbox as ["+excludeuploaddocument+"]");
			...
			if (!includesubmittedrecommendations.equals("")) {
				Reporter.log("Step 1 - Perform the operation to check the checkbox as ["+includesubmittedrecommendations+"]");
...
//					recentPopupSelectWebdriver("Print Application");
